...
genericEEGPTModel:
    ...
    Output : [n_mapped_channels, n_window_samples]  (resampled to 256 Hz, mean-centred)

Pipeline stages (each is a standalone callable):
    1. ChannelSelector      -- pick + reorder channels from canonical map
    2. Resampler            -- resample to TARGET_SAMPLE_RATE via linear interpolation
    3. Segmenter            -- cut continuous recordings into fixed-length windows
...
# Pipeline constants
TARGET_SAMPLE_RATE: int = 256        # Hz — all datasets are resampled to this
DEFAULT_WINDOW_SAMPLES: int = 256    # 1 s x 256 Hz — override via EEGPTPreprocessor
# Legacy alias kept for any external code that imported this name
TARGET_WINDOW_SAMPLES: int = DEFAULT_WINDOW_SAMPLES
...
class EEGPTPreprocessor:
    ...
    to:
        [n_trials, n_mapped_channels, target_window_samples]

    All datasets are resampled to TARGET_SAMPLE_RATE (256 Hz) using linear
    interpolation, then cropped/padded to exactly target_window_samples points.
    Default is 256 samples = 1 second at 256 Hz.
    ...
    target_window_samples : int
        Number of timepoints in the output window.
        Default: DEFAULT_WINDOW_SAMPLES (256 = 1 s at 256 Hz).
    ...
    def __init__(
        ...
        self.segmenter = Segmenter(window_samples=target_window_samples)
        ...
        self.n_timepoints: int = target_window_samples  # expose for cfg auto-setting

        logger.info(
            "EEGPTPreprocessor: %d channels, source_sfreq=%d Hz -> %d Hz, "
            "window=%d samples (%.3f s)",
            self.n_channels, source_sfreq, TARGET_SAMPLE_RATE,
            target_window_samples, target_window_samples / TARGET_SAMPLE_RATE,
        )
    ...
    def process_epochs(self, X: np.ndarray) -> torch.Tensor:
        ...
        Parameters
        ----------
        X : np.ndarray, shape [n_trials, n_raw_channels, n_raw_timepoints]

        Returns
        -------
        torch.Tensor, shape [n_trials, n_mapped_channels, target_window_samples]
        """
        x_raw = torch.FloatTensor(X)[:, self.channel_selector.dataset_indices, :]
        processed = []
        for trial in x_raw:                              # trial: [C, T_raw]
            t = self.resampler(trial)                    # [C, T_256]
            t = self.segmenter.segment_epochs(t.unsqueeze(0)).squeeze(0)  # [C, window]
            t = self.baseline(t)
            if self.clipper:
                t = self.clipper(t)
            processed.append(t)
        return torch.stack(processed, dim=0)             # [N, C, window]

    def process_continuous(
        self, x: np.ndarray
    ) -> List[torch.Tensor]:
        """
        Segment a continuous recording into fixed-length windows.
        ...
        Returns
        -------
        List of tensors each shaped [n_mapped_channels, target_window_samples]
        """
