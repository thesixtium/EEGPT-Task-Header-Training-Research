...
class MoabbDatasetLoader(BaseDatasetLoader):
    ...
    def load_all_subjects(self) -> EEGSampleDataset:
        ...
        for subject_id in all_subjects:
            ...
            # Build preprocessor once (channel layout is dataset-level constant)
            # Build preprocessor once (channel layout is dataset-level constant)
            if preprocessor is None:
                if hasattr(self._moabb_dataset, 'channel_names'):
                    raw_channel_names = self._moabb_dataset.channel_names
                else:
                    # Get channel names from the MNE epochs object
                    epochs_data, _, _ = paradigm.get_data(
                        dataset=self._moabb_dataset,
                        subjects=[subject_id],
                        return_epochs=True,
                    )
                    raw_channel_names = epochs_data.ch_names
                source_sfreq = int(
                    self._resample
