...
def make_dataset_balanced_sampler(
    ...
    """
    Return a WeightedRandomSampler that gives every dataset_id equal total
    pull on the sampling distribution, regardless of how many samples it
    contributes.

    Each sample's weight is split first across datasets (1 / n_datasets,
    divided equally among that dataset's samples), and — if
    ``balance_classes`` is True — further split across classes *within*
    that dataset (so within each dataset, classes are also balanced).

    With this sampler, a tiny dataset and a huge dataset each account for
    ~1/n_datasets of the samples drawn per epoch.
    """
    dataset_ids = [s.dataset_id for s in dataset.samples]
    labels = [s.label for s in dataset.samples]

    unique_dataset_ids = sorted(set(dataset_ids))
    n_datasets = len(unique_dataset_ids)

    ds_counts: Dict[str, int] = {}
    for did in dataset_ids:
        ds_counts[did] = ds_counts.get(did, 0) + 1

    sample_weights = torch.zeros(len(dataset.samples), dtype=torch.float)

    if balance_classes:
        class_counts_per_ds: Dict[str, Dict[int, int]] = {}
        for did, lbl in zip(dataset_ids, labels):
            class_counts_per_ds.setdefault(did, {})
            class_counts_per_ds[did][lbl] = class_counts_per_ds[did].get(lbl, 0) + 1

        for i, (did, lbl) in enumerate(zip(dataset_ids, labels)):
            n_classes_in_ds = len(class_counts_per_ds[did])
            class_count = class_counts_per_ds[did][lbl]
            sample_weights[i] = (
                1.0 / n_datasets
                / n_classes_in_ds
                / class_count
        ...
        for i, did in enumerate(dataset_ids):
            sample_weights[i] = 1.0 / n_datasets / ds_counts[did]

    logger.info(
        "make_dataset_balanced_sampler: %d datasets, each weighted to ~%.2f%% "
        "of the sampling distribution (balance_classes=%s)",
        n_datasets, 100.0 / n_datasets, balance_classes,
    )
...
class ConcatDataLoader:
    ...
    balance_mode : str — 'class' (default) balances only by class label across the
        merged pool; 'dataset' gives every dataset_id equal pull on the sampling
        distribution (and balances classes within each dataset); 'none' disables
        weighted sampling even if balanced=True.
    """
    ...
    def __init__(
        ...
        sampler = None
        if balanced:
            if balance_mode == 'dataset':
                sampler = make_dataset_balanced_sampler(combined, balance_classes=True)
            elif balance_mode == 'class':
                sampler = make_balanced_sampler(combined)
            elif balance_mode == 'none':
                sampler = None
            else:
                raise ValueError(
                    f"Unknown balance_mode '{balance_mode}' — "
                    f"expected 'class', 'dataset', or 'none'"
                )
        ...
        logger.info(
            "ConcatDataLoader: %d datasets merged → %d total samples, "
            "%d channels, %d steps/epoch",
            len(datasets), self.n_samples,
            len(self.channel_names), self.steps_per_epoch,
        )
...
class MultiChannelDataLoader:
    ...
    def __init__(
        ...
        logger.info(
            "MultiChannelDataLoader: %d datasets, %d total steps/epoch",
            len(dataset_loaders), self.steps_per_epoch,
        )
